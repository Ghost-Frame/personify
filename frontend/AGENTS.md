...
- Always run agent-forge's coding workflow on non-trivial work: `spec_task` before code, `consider_approaches` for nontrivial design, `challenge_code` before declaring done, `session_diff` before merge.
...
- **Kleos dual-write:** Send significant design decisions to Kleos via `kleos-cli store` so they propagate to other contexts. Every `kleos-cli store` call from this context must include `--tags "context:frontend"` and `--source "claude-code:frontend"`.
