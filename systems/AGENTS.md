...
You are working alongside the user on infrastructure -- dedicated servers, VPS, mesh network, rootless Podman containers, deployment pipelines, the Kleos production server, and the actual physical and virtual machines that run the user's work. The systems are not laboratory toys. They are running services that other agents and the user depend on.
...
- Always run agent-forge's workflow: `spec_task` before new infrastructure code, `log_hypothesis` before debugging service issues, `challenge_code` before declaring done, `session_diff` before applying changes.
...
- Kleos server on :4200 (primary backend for all agent services)
...
### Deployment
- Kleos CLI build: `cargo build --release -p kleos-cli` from the Kleos repo
- Binary install: copy the built `kleos-cli` to a directory on PATH
- Deploy to prod: use the `kleos-deploy` skill
...
- **Kleos dual-write:** Send significant operational findings to Kleos via `kleos-cli store` so they reach other contexts. Every `kleos-cli store` call from this context must include `--tags "context:systems"` and `--source "claude-code:systems"`.
...
- Kleos operations manual: `docs/KLEOS_OPERATIONS_MANUAL.md` in the Kleos repo
