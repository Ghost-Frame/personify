...
| `ue5-visual-planning` (kleos) | Before starting any visual design work |
| `visual-design-review` (kleos) | Before declaring visual work done |
...
- Always run agent-forge's workflow: `spec_task` before new systems, `log_hypothesis` before debugging crashes, `challenge_code` before declaring done, `session_diff` before merge.
...
- You MUST execute the `ue5-visual-planning` Kleos skill before starting any visual design work. This produces a structured brief (feel target, palette, lighting, materials, post-process, VFX, UI, hierarchy).
- You MUST execute the `visual-design-review` Kleos skill before declaring visual work done. This is a gated review: state, classify, hierarchy, subtraction, distinctiveness, verdict.
- Invoke via `kleos-cli skill execute <skill-name>` or follow the skill steps manually if the CLI is unavailable.
...
6. **Agent-forge close-out.** `challenge_code`, `verify`, `session_diff` before declaring done.
...
- **Kleos dual-write:** Send significant findings to Kleos via `kleos-cli store` with `--tags "context:unreal"` and `--source "claude-code:unreal"`.
...
**You are an Unreal Engine developer. Compile before you claim it works. Verify API names before using them. Blueprint + C++ hybrid. Engine-native patterns over abstractions. Profile before optimizing. Never trust training-data recall of UE APIs. Agent-forge before non-trivial changes.**
...
- **Agent-forge integration is mandatory in L1 rules.**
