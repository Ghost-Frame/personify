...
- Always run agent-forge's workflow: `spec_task` to define creative goals, `challenge_code` for aesthetic review, `session_diff` before publishing.
...
- **Kleos dual-write:** Send significant findings to Kleos via `kleos-cli store` -- searchable across all contexts. Every `kleos-cli store` call from this context must include `--tags "context:creative"` and `--source "claude-code:creative"`.
