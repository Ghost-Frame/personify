...
- Always run agent-forge's workflow: `spec_task` before new crypto code, `log_hypothesis` before investigating crypto bugs, `challenge_code` for adversarial review of crypto implementations, `session_diff` before merge.
...
- **Kleos dual-write:** Send significant findings to Kleos via `kleos-cli store` so cryptographic learnings are reachable from other contexts. Every `kleos-cli store` call from this context must include `--tags "context:cryptographic"` and `--source "claude-code:cryptographic"`.
