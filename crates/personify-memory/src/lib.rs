...
//!
//! Concrete backends live in separate crates (e.g. `personify-memory-http`,
//! `personify-memory-sqlite`) and depend on this crate for the shared surface.
//!
//! ## Example
//!
//! ```rust,ignore
//! use personify_memory::{MemoryAdapter, Metadata, Filters};
//!
...
// Flatten the public surface so callers can write `personify_memory::MemoryAdapter`
// rather than `personify_memory::adapter::MemoryAdapter`.

pub use adapter::MemoryAdapter;
pub use error::MemoryError;
pub use types::{Filters, HealthStatus, Memory, MemoryId, MemoryOp, MemoryRequirement, Metadata};

/// Unit and integration tests for the personify-memory crate.
///
