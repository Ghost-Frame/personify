...
- Always run agent-forge's workflow: `spec_task` before new pipelines, `log_hypothesis` before debugging pipeline failures, `challenge_code` before declaring done, `session_diff` before merge.
...
- **Kleos dual-write:** Send significant findings to Kleos via `kleos-cli store` -- searchable across all contexts. Every `kleos-cli store` call from this context must include `--tags "context:data"` and `--source "claude-code:data"`.
