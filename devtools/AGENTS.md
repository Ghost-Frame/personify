...
- Always run agent-forge's workflow: `spec_task` before new tools, `consider_approaches` for design decisions, `challenge_code` before declaring done, `session_diff` before merge.
...
### AST and parsing
- tree-sitter 0.24 for AST parsing (Code Crucible)
...
- agent-forge as exemplar: `--input`/`--output` JSON files, rusqlite for state persistence
...
- **Kleos dual-write:** Send significant findings to Kleos via `kleos-cli store` -- searchable across all contexts. Every `kleos-cli store` call from this context must include `--tags "context:devtools"` and `--source "claude-code:devtools"`.
